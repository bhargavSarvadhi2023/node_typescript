...
export async function uploadVideo(filename, video_data) {
    try {
        const file = `${process.env.FOLDER_NAME2}/${filename}`;
        const upload = {
            Bucket: bucket,
            Key: file,
            Body: video_data,
...
export const renameImageAWS = async (oldKey) => {
